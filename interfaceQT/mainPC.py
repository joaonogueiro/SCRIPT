...
class MainWindow(QMainWindow):
    def __init__(self, pi, inputs, outputs):
        ...
        self.ui.btn_recipePage.clicked.connect(lambda: self.ui.stackedWidget_Pages.setCurrentWidget(self.ui.recipePage))
        # IO monitor Pop-up:
    ...
    def convert_qt_cv(self, qt_img): # Convert QT format to OpenCV format for visualization
        h, w, ch = qt_img.shape
        bytes_per_line = ch * w
        qImg = QImage(qt_img.data, w, h, bytes_per_line, QImage.Format_RGB888).rgbSwapped()
    ...
    def autoModeLoop(self):
        self.btn_START.setEnabled(False)
        self.btn_STOP.setEnabled(True)
        self.lb_estado.setText("Aguardando sensores")

        def check_sensors():
            while not self.STOP_state:
                if self.B01_doorLeft.is_pressed and self.B02_doorRight.is_pressed and self.B03_board.is_pressed:
                    self.analiseProcess()
                    while self.B03_board.is_pressed and not self.STOP_state:
                        time.sleep(0.1)
                    self.wait_for_board_removal()
                time.sleep(0.1)

        self.sensor_thread = threading.Thread(target=check_sensors)
        self.sensor_thread.start()

    def wait_for_board_removal(self):
        while not self.STOP_state:
            if not self.B03_board.is_pressed:
                time.sleep(5)
                if not self.B03_board.is_pressed:
                    self.lb_estado.setText("A Processar")
                    self.analiseProcess()
    ...
    def analiseProcess(self):
        ...
        try:
            ...
            cv2.imwrite("FINALImg.png", final_result)  #NOTE Debug 
            resultImageQT = self.convert_qt_cv(final_result)
            self.lb_resultImage.setPixmap(resultImageQT)
            self.lb_resultImage.setScaledContents(True)
            self.tf = time.time()
            self.cycle_time = round(self.tf - self.ti, 2)
            self.lb_cicleTime.setText(str(self.cycle_time))

            # NOTE RESULTADOS
            # Adiciona o tempo do ciclo ao ficheiro cycleTime.txt
            with open("cycleTime.txt", "a") as f:
                f.write(f"{self.cycle_time}\n")

        except Exception as e:
            time_str = self.DateTime.toString('hh:mm:ss')
            self.textEdit.setTextColor(QtGui.QColor("red"))
            self.textEdit.append(f"{time_str}   ERRO001 - Erro no Processamento de imagem")
            self.textEdit.setTextColor(QtGui.QColor("black"))
            pass
