...
class Algoritm:
    ...
    def undistort(self, image):
        ...
        # rotation_matrix = cv2.getRotationMatrix2D((w/2, h/2), 0.8, 1) # Angle is negative for clockwise rotation
        ...
        cv2.imshow("Result ", self.resizeImage(image_Undistort_crop))
        cv2.waitKey(0)
    ...
    def main(self, imageWhite, imageUV):
        ...
        self.sceneWhiteTemplt = copy.deepcopy(self.sceneWhiteGRAY)

        ### -> Template Image <- ### 
        self.template_img = cv2.imread("static/Template/template_8.png")
        # self.template_img = cv2.imread("../interfaceQT/testImages/cropped_image.png")
        ...
        # while self.thresh < self.threshVAL:
            # idx +=1
        for idx, m in enumerate(range(0, self.PCB_numb)):
            top_left, bottom_right, self.threshVAL = self.template_matching(self.template_gray,  self.sceneWhiteTemplt)
            cv2.rectangle(self.sceneWhiteTemplt, top_left, bottom_right, (0, 0, 0), -1)
            cv2.putText(self.sceneWhiteTemplt, str(idx), (top_left[0]+150, top_left[1]+150), cv2.FONT_HERSHEY_SIMPLEX , 2, (255, 255, 255), 5, cv2.LINE_AA) 
            self.template_gray = self.img_crop(self.sceneWhiteGRAY, top_left[1], bottom_right[1],top_left[0], bottom_right[0])
            cv2.imwrite(f"../interfaceQT/testImages/cropedImag/CropImages_{idx}.png", self.template_gray)
            if idx == 1:
                self.template_gray = self.img_crop(self.sceneWhiteGRAY, top_left[1], bottom_right[1],top_left[0], bottom_right[0])
                # cv2.imshow("TEMPLATE", self.template_gray)
                # cv2.waitKey(0)
        ...
        cv2.imshow(f"Result {idx}", self.resizeImage( self.sceneWhiteTemplt))
        cv2.waitKey(0)
        ...
        # self.templateSIFT = cv2.imread("../interfaceQT/testImages/cropped_image.png")

        self.templateSIFT_GRAY = cv2.cvtColor(self.templateSIFT, cv2.COLOR_BGR2GRAY) 
        self.templateSIFT_GRAY = self.template_gray
        #####################################
        ...
        for idx, info in self.dict_infoPCB_Sorted.items():
            ...
            kp1, kp2, good_matches, imgMatch = self.descritorSIFT(self.templateSIFT_GRAY, self.PCB_cropWhiteGRAY, 0.75)
            cv2.imshow(f"MATCHES_{idx}", imgMatch)
            cv2.waitKey(0)
            ...
            x1, y1 = dst[0,0], dst[0,1]
            x2, y2 = dst[1,0], dst[1,1]
            x3, y3 = dst[2,0], dst[2,1]
            x4, y4 = dst[3,0], dst[3,1]

            # # Draw points at each vertex of the bounding box
            # points = [(x1, y1), (x2, y2), (x3, y3), (x4, y4)]
            # for idx, (x, y) in enumerate(points):
            #     cv2.circle(self.imageWhite_Undst, (int(x), int(y)), radius=5, color=(0, 0, 0), thickness=-1)
            #     cv2.putText(self.imageWhite_Undst, f'Point {idx}', (int(x) + 10, int(y) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 1, cv2.LINE_AA)

            # cv2.imshow("RESULT",(self.imageWhite_Undst))
            # cv2.waitKey(0)
            ...
            scene_img = cv2.polylines(self.PCB_cropWhite, [np.int32(dst)], True, (255, 255, 255), 1, cv2.LINE_AA)
            cv2.imshow(f'{idx} SIFT', scene_img)
            ##########################################################################################################

            tol = 0
            self.PCB_cropedUV = self.img_crop(self.PCB_cropUV_aligned, round(min(y1, y4))-tol, round(max(y2, y3))+tol, round(min(x1, x2))-tol, round(max(x3, x4))+tol) #NOTE alterei o round(max(y2, y3))+tol)) para round(min(y2, y3))+tol))
            cv2.imshow(f'{idx} Croped UV', self.PCB_cropedUV)
            cv2.imwrite(f'ImageUV_{idx}.png', self.PCB_cropedUV)
...
# NOTE Debug:
algoritm = Algoritm()
imageWhite = cv2.imread('testImages/imagesAcq/Image1_31.png')
imageUV = cv2.imread('testImages/imagesAcq/Image2_31.png')
